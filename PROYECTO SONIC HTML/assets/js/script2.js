alert("Estas a punto de jugar SONIC HTML, Asegurate de ajustar el zoom de tu navegador para una mejor experiencia, puedes intentar con un 80%")
//FUNCION SELECCION DE COLOR
alert("SELECCIONA EL COLOR DE SONIC")

if (document.getElementById('css-select')) {
    document.getElementById('css-select').addEventListener('change', function() {
        var selectedCSS = this.value;
        var link = document.createElement('link');
        link.rel = 'stylesheet';
        link.type = 'text/css';
        link.href = selectedCSS;
...
function iniciarJuego(){
    let botonSonicJugador = document.getElementById('css-select')
    botonSonicJugador.addEventListener('change', seleccionarSonicJugador)    
}

function seleccionarSonicJugador() {
    if (document.getElementById('style1.css').selected){
        alert("Has seleccionado a Sonic Clasico")
    }
    else if (document.getElementById('style2.css').selected){
        alert("Has seleccionado a Sonic Verde")
    }
    else{
...
let song= new Audio("greenHill.mp3");
